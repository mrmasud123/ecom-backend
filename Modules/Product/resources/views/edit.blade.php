...
@section('vendor-scripts')
    @vite(['Modules/Product/resources/assets/js/product-edit.js']);
@endsection

@section('content')

    <x-common.page-breadcrumb :pageTitle="[['name' => 'Products', 'link' => route('product.index')], ['name' => 'Edit Product', 'link' => '#']]" />

    <form id="editProductForm" action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-3 items-start">
            ...
            <div class="space-y-6 xl:col-span-2 order-1">
                ...
                <div class="bg-white dark:bg-white/5 rounded-2xl shadow-lg p-6">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input type="text" id="name" name="name" required value="{{ old('name', $product->name) }}" placeholder="e.g. Everyday Canvas Tote"
                        ...
                        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                            <div>
                                ...
                                <input type="text" id="sku" name="sku" required value="{{ old('sku', $product->sku) }}" placeholder="TOTE-CAN-001"
                            ...
                            <div>
                                ...
                                <input type="text" id="slug" name="slug" value="{{ old('slug', $product->slug) }}" placeholder="everyday-canvas-tote"
                                       class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-400 focus:outline-none focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:text-white">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="short_description" name="short_description" maxlength="255" value="{{ old('short_description', $product->short_description) }}" placeholder="One line for listings and search results"
                                   class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-400 focus:outline-none focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:text-white">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="6" placeholder="Materials, fit, care instructions — whatever helps someone decide."
                                      class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-400 focus:outline-none focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:text-white">{{ old('description', $product->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-white/5 rounded-2xl shadow-lg p-6">
                    ...
                    <p class="mb-5 text-sm text-gray-500 dark:text-gray-400">The first image is the product's cover. Uploading new images adds to, not replaces, existing ones.</p>

                    {{-- Existing images --}}
                    @if($product->getMedia('images')->isNotEmpty())
                        <div id="existingImageGrid" class="mb-4 grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6">
                            @foreach($product->getMedia('images') as $media)
                                <div class="group relative aspect-square overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700" data-media-id="{{ $media->id }}">
                                    <img src="{{ $media->getUrl() }}" alt="" class="h-full w-full object-cover">
                                    <button type="button" data-media-id="{{ $media->id }}"
                                            class="remove-existing-image absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-black/60 text-white opacity-0 transition group-hover:opacity-100 hover:bg-red-600">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                                        </svg>
                                    </button>
                                    @if($loop->first)
                                        <span class="absolute bottom-1 left-1 rounded bg-black/60 px-1.5 py-0.5 text-[10px] font-medium text-white">Cover</span>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                        {{-- Hidden inputs populated by JS when an existing image's remove button is clicked --}}
                        <div id="removedMediaIds"></div>
                    @endif
                ...
                <div class="bg-white dark:bg-white/5 rounded-2xl shadow-lg p-6">
                    ...
                    <p class="mb-5 text-sm text-gray-500 dark:text-gray-400">
                        @if($product->has_variants)
                            Base pricing shown here is a fallback. Each variant below can override it.
                        @else
                            What the customer pays, what it costs you, and what "on sale" looks like.
                        @endif
                    </p>
                    ...
                    <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" step="0.01" min="0" id="price" name="price" required value="{{ old('price', $product->price) }}" placeholder="0.00"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-gray-400">৳</span>
                                <input type="number" step="0.01" min="0" id="compare_price" name="compare_price" value="{{ old('compare_price', $product->compare_price) }}" placeholder="0.00"
                                       class="h-11 w-full rounded-lg border border-gray-300 bg-transparent pl-8 pr-4 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-400 focus:outline-none focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:text-white">
                            </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-gray-400">৳</span>
                                <input type="number" step="0.01" min="0" id="cost_price" name="cost_price" value="{{ old('cost_price', $product->cost_price) }}" placeholder="0.00"
                                       class="h-11 w-full rounded-lg border border-gray-300 bg-transparent pl-8 pr-4 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-400 focus:outline-none focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:text-white">
                            </div>
                ...
                {{-- Inventory (hidden entirely when product has variants — stock is per-variant instead) --}}
                <div id="inventoryCard" class="bg-white dark:bg-white/5 rounded-2xl shadow-lg p-6 {{ $product->has_variants ? 'hidden' : '' }}">
                    ...
                    <div class="mb-5 flex items-center justify-between rounded-lg border border-gray-200 px-4 py-3 dark:border-gray-700">
                        ...
                        <label class="relative inline-flex cursor-pointer items-center">
                            <input type="checkbox" name="track_quantity" id="track_quantity" value="1" {{ old('track_quantity', $product->track_quantity) ? 'checked' : '' }} class="peer sr-only">
                    ...
                    <div id="quantityFields" class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="number" min="0" id="quantity" name="quantity" value="{{ old('quantity', $product->quantity) }}" placeholder="0"
                                   class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-400 focus:outline-none focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:text-white">
                        </div>
                        <div class="flex items-center gap-3 pt-7">
                            <input type="checkbox" name="allow_backorder" id="allow_backorder" value="1" {{ old('allow_backorder', $product->allow_backorder) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:border-gray-700">
                ...
                <div id="variantsCard" class="bg-white dark:bg-white/5 rounded-2xl shadow-lg p-6 {{ $product->has_variants ? '' : 'hidden' }}">
                            ...
                            @forelse($attributes ?? [] as $attribute)
                                <label class="attribute-checkbox flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                    <input type="checkbox"
                                        ...
                                        {{ $selectedAttributeIds->contains($attribute->id) ? 'checked' : '' }}
                                    >
                                    {{ $attribute->name }}
                                </label>
                            @empty
                ...
                <div class="bg-white dark:bg-white/5 rounded-2xl shadow-lg p-6">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input type="text" id="meta_title" name="meta_title" maxlength="255" value="{{ old('meta_title', $product->meta_title) }}" placeholder="Defaults to product name if left blank"
                                   class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-400 focus:outline-none focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:text-white">
                        </div>
                        <div>
                            ...
                            <textarea id="meta_description" name="meta_description" rows="3" maxlength="255" placeholder="One or two sentences shown under the title in search results"
                                      class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-400 focus:outline-none focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:text-white">{{ old('meta_description', $product->meta_description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="order-2 xl:order-1 bg-white dark:bg-gray-900 rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-800">
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="flex items-center justify-between rounded-lg border border-gray-200 px-4 py-3 dark:border-gray-700">
                            ...
                            <label class="relative inline-flex cursor-pointer items-center">
                                <input type="checkbox" name="has_variants" id="has_variants" value="1" {{ old('has_variants', $product->has_variants) ? 'checked' : '' }} class="peer sr-only">
                                <div class="h-6 w-11 rounded-full bg-gray-200 peer-checked:bg-green-600 transition-colors "></div>
                                <div class="absolute left-1 top-1 h-4 w-4 rounded-full bg-white transition-transform peer-checked:translate-x-5"></div>
                            </label>
                        </div>
                        @if($product->has_variants && $product->variants->isNotEmpty())
                            <p class="text-xs text-amber-600 dark:text-amber-400">
                                This product has {{ $product->variants->count() }} variant(s). Turning this off will require removing them first.
                            </p>
                        @endif
                    </div>
                </div>
