{{-- Modules/Product/resources/views/brand/edit.blade.php --}}
@extends('layouts.app')

@section('vendor-scripts')
    @vite(['Modules/Product/resources/assets/js/brand-edit.js']);
@endsection
...
@section('content')
    ...
    <x-common.page-breadcrumb :pageTitle="[
        ...
        ['name' => 'Edit Brand', 'link' => '#'],
    ]" />

    <div class="bg-white dark:bg-white/5 rounded-2xl shadow-lg p-6 w-full">

        <div class="mb-6">
            <h2 class="text-xl font-semibold dark:text-white">Edit Brand</h2>
        ...
        <form id="brandEditForm" enctype="multipart/form-data"
              data-update-url="{{ route('brands.update', $brand->id) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $brand->description }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <label for="logo" id="logoDropzone"
                        ...
                        <div id="logoPreviewWrapper" class="h-16 w-16 rounded-lg border border-gray-200 dark:border-gray-700 flex items-center justify-center overflow-hidden bg-white dark:bg-gray-800 shrink-0">
                            @if($brand->logo)
                                <img src="{{ asset('storage/' . $brand->logo) }}" class="h-full w-full object-cover">
                            @else
                        ...
                        <div class="flex-1 min-w-0">
                            <p id="logoFileName" class="text-sm font-medium {{ $brand->logo ? 'text-gray-700 dark:text-gray-300' : 'text-gray-500 dark:text-gray-400' }} truncate">
                                {{ $brand->logo ? basename($brand->logo) : 'Click or drag an image to upload' }}
                            </p>
                            <p id="logoFileMeta" class="text-xs text-gray-400 mt-0.5">
                                {{ $brand->logo ? 'Current logo — click to replace' : 'PNG, JPG or WEBP — max 2MB' }}
                            </p>
                        </div>

                        <button type="button" id="logoRemoveBtn"
                                class="{{ $brand->logo ? '' : 'hidden' }} shrink-0 p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-red-500">
                    ...
                    {{-- Signals intent to backend: has an existing logo been explicitly removed with no replacement? --}}
                ...
                <div class="md:col-span-2 flex items-center gap-3">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ $brand->is_active ? 'checked' : '' }} class="sr-only peer">
